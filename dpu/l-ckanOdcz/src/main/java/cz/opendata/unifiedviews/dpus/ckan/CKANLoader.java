...
//		    	String dspatial = executeSimpleSelectQuery("SELECT ?spatial WHERE {<" + distribution + "> <"+ DCTERMS.SPATIAL + "> ?spatial }", "spatial");
	            ...
	            {
		            if (config.isGenerateVirtuosoTurtleExampleResource()) {
				    	...
				    	if (!dissued.isEmpty()) exTurtle.put("created", dissued);
				    	if (!dmodified.isEmpty()) exTurtle.put("last_modified", dmodified);
		            ...
		            if (config.isGenerateExampleResource()) {
				    	...
				    	if (!dissued.isEmpty()) exHTML.put("created", dissued);
				    	if (!dmodified.isEmpty()) exHTML.put("last_modified", dmodified);
