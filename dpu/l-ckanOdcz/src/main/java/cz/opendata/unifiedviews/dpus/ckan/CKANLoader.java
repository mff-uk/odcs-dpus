        ...
        try {
			...
			for (String distribution: distributions) {
	            ...
	            for (String example: examples)
	            {
		            ...
		            try {
						if (sparqlEndpoint.isEmpty()) {
							exTurtle.put("url", example);
						} else exTurtle.put("url", sparqlEndpoint + "?query=" + URLEncoder.encode("DESCRIBE <" + example + ">", "UTF-8") + "&output=" + URLEncoder.encode("text/turtle","UTF-8") );
