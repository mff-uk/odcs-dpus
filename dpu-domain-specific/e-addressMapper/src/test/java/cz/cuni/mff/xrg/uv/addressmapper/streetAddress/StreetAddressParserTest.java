...
public class StreetAddressParserTest {
    ...
    @Test
    public void houseNumberWithLetter() throws WrongAddressFormatException {
        StreetAddress addr = parser.parse("Křižíkova 23/48b");
        
        Assert.assertNull(addr.getTownName());
        Assert.assertEquals("48b", addr.getHouseNumber());
        Assert.assertEquals("23", addr.getLandRegistryNumber());
        Assert.assertEquals("Křižíkova", addr.getStreetName());        
    }
}
