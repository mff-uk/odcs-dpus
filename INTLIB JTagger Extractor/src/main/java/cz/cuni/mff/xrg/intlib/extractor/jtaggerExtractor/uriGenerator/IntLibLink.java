...
//        }
        ...
        //Logger.getLogger("intlib").log(new Work("cs", WorkType.ACT, 1994, "5", null, null, null));
        String inputDir = "";
        String inputFile = "";
        String outputDir = "";
        String outputFile = "";
        ...
        if (!inputFile.isEmpty()) {
            ...
            if (
                ...
                IntLibLink.help("Input file '" + inputFile + "' does not exist");
                return;
            }
        } else {
        ...
        if (!config.exists()) {
            IntLibLink.help("Configuration file '" + configFile + "' does not exist");
            return;
        }
        ...
        if (!inputDir.isEmpty()) {
            ...
            for (File inputXML: dirList) {
                ...
                logger.info("Parsing file {0}/{1}: {2}", new Object[] { Integer.toString(count), Integer.toString(dirList.length), inputXML.getName()});
//                Logger.getLogger(IntLibLink.class.getName()).log(Level.INFO, "");

                doc.transform(inputXML.getAbsolutePath() , outputXML.getAbsolutePath());
...
//                Logger.getLogger("intlib").log(Level.INFO, "");
                logger.info("Recognized:" + (Work.recognized - lastrec));
                 ...
                 logger.info("Last shortcut used:" + (Work.lastShortcutUsed - lastshortcut));
                 logger.info("Last number used:" + (Work.lastNumberUsed - lastnumber));
                 logger.info("Shortcuts trimmed from start:" + (Work.startShortTrimmed - laststarttrimmed));
                 logger.info("Shortcuts trimmed from end:" + (Work.endShortTrimmed - lastendtrimmed));
                 logger.info("Až:" + (Work.az_expanded - lastaz));
                 logger.info("Expressions trimmed from end:" + (Work.expressionEndTrimmed - lastexpendtrimmed));
                 logger.info("Institutions:" + (Work.institutions - lastinstit));
                 logger.info("Recognized total: " + (Work.recognized + Work.institutions - lastrec - lastinstit));
                 logger.info("Unrecognized total: " + (Work.nedostatecne + Work.missing + Work.unrecognized - lastunrec - lastmiss - lastnedost));
                 logger.info("Success rate: " 
                + (int)((double)(Work.recognized + Work.institutions - lastrec - lastinstit)/(double)(Work.recognized + Work.institutions - lastrec - lastinstit + Work.nedostatecne + Work.missing + Work.unrecognized - lastunrec - lastmiss - lastnedost)*100) 
                + "%");
                 logger.info("Realistic (- institutions) success rate: " 
                + (int)((double)(Work.recognized - lastrec)/(double)(Work.recognized - lastrec + Work.nedostatecne + Work.missing + Work.unrecognized - lastunrec - lastmiss - lastnedost)*100) 
                + "%");
            }
        }
        ...
        if (!inputFile.isEmpty()) {
         ...
         logger.info("Recognized total: " + (Work.recognized + Work.institutions));
         logger.info("Unrecognized total: " + (Work.nedostatecne + Work.missing + Work.unrecognized));
         logger.info("Success rate: " 
                + (int)((double)(Work.recognized + Work.institutions)/(double)(Work.recognized + Work.institutions + Work.nedostatecne + Work.missing + Work.unrecognized)*100) 
                + "%");
         logger.info("Realistic success rate: " 
                + (int)((double)(Work.recognized)/(double)(Work.recognized + Work.nedostatecne + Work.missing + Work.unrecognized)*100) 
                + "%");
                
        
    }

//    public static void processFiles(String file, String output, org.slf4j.Logger logger) {
